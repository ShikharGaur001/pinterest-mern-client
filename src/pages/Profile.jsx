...
const Profile = () => {
  ...
  const { user, selectedProfile, myProfile, isLoading, isError, message } =
  ...
  const handleFollow = () => {
    dispatch(followUser(selectedProfile?._id)).then(() => {
      dispatch(getProfile(userName));
      dispatch(getCurrentUser());
    });
  };
      ...
      <>
        ...
        <div className="w-screen py-8 px-20">
          ...
          <span className="w-full mt-2 flex items-center justify-center font-light">
            {selectedProfile?.followers.length} followers •{" "}
    ...
    return (
      <>
        ...
        <div className="w-screen py-8 px-20">
          ...
          <div className="w-full flex mt-2 items-center justify-center gap-4">
            <button className="hover:bg-zinc-100 rounded-full h-12 w-12 p-3.5 -ml-12">
              <img
            ...
            <button
              ...
              className={`${
                myProfile?.following?.some(
                  (user) => user._id === selectedProfile?._id
                )
                  ? "bg-black"
                  : "bg-pinterest"
              } text-white rounded-full px-5 py-3 font-semibold`}
            >
              {myProfile?.following?.some(
                (user) => user._id === selectedProfile?._id
              )
